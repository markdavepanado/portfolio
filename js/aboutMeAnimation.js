...
var amLineVertical = document.getElementsByClassName(
  "about-me-line-vertical"
)[0];
var personalInformation = document.getElementsByClassName(
...
var firstLineHorizontal = personalDetails.getElementsByClassName(
  "line-horizontal"
)[0];
var awardDetails = Array.from(document.getElementsByClassName("award-details"));
var programDetails = Array.from(
  document.getElementsByClassName("program-details")
);
...
function slideUpVLine() {
  ...
  var translateYpercentage = Math.round(
    (translateYpixels / aboutMeHeight) * 100
  );
...
function slideRightFLhorizontal() {
  ...
  var firstLineHorizontalWidth = Math.round(
    firstLineHorizontal.getBoundingClientRect().width
  );
  var translateXpixels = getCurrentTranslateX(firstLineHorizontal);
  var translateXpercentage =
    Math.round((translateXpixels / firstLineHorizontalWidth) * 100) + 1;
...
function fadeInImg() {
  ...
  var value = parseFloat(
    window.getComputedStyle(pdImg).getPropertyValue("opacity")
  );
  var instance = window.setInterval(function () {
    if (value > 1) {
      window.clearInterval(instance);
...
function slideLeftPI() {
  ...
  var personalDetailsWidth =
    Math.round(personalDetails.getBoundingClientRect().width) +
    bodyPaddingXaxis +
    Math.round(pdImg.getBoundingClientRect().width) +
    piPaddingXaxis;
  var translateXpixels = getCurrentTranslateX(personalInformation);
  var translateXpercentage =
    Math.round((translateXpixels / personalDetailsWidth) * 100) + 1;
...
function slideRightPDLineHorizontal(element) {
  ...
  var pdLineHorizontalWidth = Math.round(
    pdLineHorizontal.getBoundingClientRect().width
  );
  var translateXpixels = getCurrentTranslateX(pdLineHorizontal);
  var translateXpercentage =
    Math.round((translateXpixels / pdLineHorizontalWidth) * 100) + 1;
...
function fadeInProgramInformation(element) {
  var programInformation = element.getElementsByClassName(
    "program-information"
  )[0];
  programInformation.style.transition = "initial";
  var increment = 0.01;
  var value = parseFloat(
    window.getComputedStyle(programInformation).getPropertyValue("opacity")
  );
  var instance = window.setInterval(function () {
    if (value > 1) {
      window.clearInterval(instance);
...
function clearProgramDetails(element) {
  element.getElementsByClassName("line-horizontal")[0].style.transition =
    "all 500ms";
  element.getElementsByClassName("program-information")[0].style.transition =
    "all 250ms";
  element.getElementsByClassName("line-horizontal")[0].style.transform =
    "translateX(-110%)";
...
function slideRightADLineHorizontal(element) {
  ...
  var adLineHorizontalWidth = Math.round(
    adLineHorizontal.getBoundingClientRect().width
  );
  var translateXpixels = getCurrentTranslateX(adLineHorizontal);
  var translateXpercentage = Math.round(
    (translateXpixels / adLineHorizontalWidth) * 100
  );
...
function fadeInAwardInformation(element) {
  ...
  var increment = 0.01;
  var value = parseFloat(
    window.getComputedStyle(awardInformation).getPropertyValue("opacity")
  );
  var instance = window.setInterval(function () {
    if (value > 1) {
      window.clearInterval(instance);
...
function clearAwardDetails(element) {
  element.getElementsByClassName("line-horizontal")[0].style.transition =
    "all 500ms";
  element.getElementsByClassName("award-information")[0].style.transition =
    "all 250ms";
  element.getElementsByClassName("line-horizontal")[0].style.transform =
    "translateX(-110%)";
...
function getPaddingXaxis(element) {
  return (
    ...
    parseFloat(window.getComputedStyle(element).paddingRight.replace(/\D/g, ""))
  );
}
